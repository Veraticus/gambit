...
- Make the minimal edit. Preserve the code, structure, and names around your change; add no guard, fallback, retry, or handling for a case the brief does not name. A diff larger than the task needs is a defect the orchestrator's gate returns to you, not thoroughness.
