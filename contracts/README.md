...
The `scout`, `steelman`, `finder`, and `verifier` roles are read-only. Only `worker` and `escalation` may change owned files. The `test-runner` may write scratch state only.
