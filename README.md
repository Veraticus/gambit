...
3. **Cheapest capable model first.** Work starts on the entry rung of its role. It moves one rung up only when a gate record says NOT DONE. No agent chooses its own rung.
...
8. **Read-only roles never write.** Scout, steelman, finder, and verifier inspect and report. Only a worker changes the tree, and only the orchestrator commits.
...
3. **Build each task until good.** A worker on the entry rung works in an isolated workspace under the worker contract and a brief. The orchestrator writes the gate record, binary and itemized against the contract, noting any Premise the work bears on; a false Premise triggers its clause. NOT DONE sends the task one rung up with it. If the top rung fails, the task is re-decomposed once; descendants climb but never split. If a descendant fails at the top rung, the lineage is a **gap**: its work is not integrated; everything not depending on it continues.
