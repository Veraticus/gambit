---
name: writing-skills
description: "Validates that a new or modified skill actually changes model behavior before it ships. Use when creating a new skill, modifying an existing skill, writing or rewriting a SKILL.md file, auditing a skill's description for discoverability, or when user mentions \"create a skill\", \"write a skill\", \"new skill\", \"modify skill\", \"improve skill\", \"edit the skill\"."
---
...
Also read `references/codex-skill-guidance.md` completely before writing: it carries the Codex skill anatomy, discoverability rules, and the `agents/openai.yaml` metadata split.
...
```
    ...
    [What Codex does wrong unaided or with the current skill]
...
Academic "does Codex follow the rule?" scenarios prove nothing — the model recites the skill back.
...
(`codex-contracts/VALIDATION.md`, 2026-07-24), an emphatic absolute ("NEVER… NO EXCEPTIONS") and a
...
Then write the results into `codex-contracts/VALIDATION.md`: exact prompts/artifact locations, conditions,
...
Present the complete evaluation results in the root checkpoint, then use `SessionPlanWrite` only to replace the complete ordered plan and mark the single evaluation wave completed. Individual evaluation cases remain transcript results, not plan steps.
