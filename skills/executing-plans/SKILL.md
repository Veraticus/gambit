...
Read `contracts/models.md` and resolve every dispatched role through the registry at `~/.claude/gambit/models.json`. Look up the role, select its entry or gate-required next rung, and select the rung's dispatch target, using the read-only variant for a read-only role. Use the dispatch operation with the role's contract by absolute path and its complete brief as text. Resolve contract paths from the current installation. The `worker` and `escalation` roles use `contracts/worker.md`; `scout` uses `contracts/scout.md`. Missing or unresolvable registry entries become recorded gaps, followed by the report and end operation, never an invented target.
...
Worker returns DONE, DONE_WITH_CONCERNS, NEEDS_CONTEXT, and BLOCKED are evidence for this gate, not terminal outcomes. Supply missing context from the tree or contract. A concern counts only if evidence establishes a contract defect. An unsatisfied task receives NOT DONE regardless of the worker's return label; a reasoning failure is no reason to repeat an unchanged entry-rung dispatch.

Route each gate deterministically:

1. **DONE:** retain the complete accepted change set for integration.
2. **NOT DONE below the top:** resolve `escalation` for exactly the next rung on this task's ladder. Carry the gate record, complete brief, contract path, and current work into that dispatch. Never move down, skip a rung, or let an agent choose its rung. Do not split before a top-rung failure.
3. **Top-rung failure of an unsplit task:** re-decompose it exactly once into complete smaller tasks covering the same unmet Requirements. Record the parentage and consumed decomposition. Record the complete conditional ladder for every descendant before dispatch: each starts at the entry rung; NOT DONE at entry sends it to the next rung; successive NOT DONE records advance it one rung at a time through the top; a top-rung failure becomes a gap. Descendants never split.
4. **Top-rung failure of a descendant:** mark the exhausted lineage as a gap. Commit its workspace to `gap/<task-slug>` and retain its final gate record. Exclude the lineage's work from integration. Continue every independent task; a gap does not end executable work elsewhere.
